...
- Laubige Vegetation (Kronen, Büsche, Gras) wird mit `gras` gezeichnet, von Natur aus rotes oder goldenes Laub mit `laub`. Werkzeugköpfe entstehen in `stein` und werden über die Materialstufen-Zeilen umgefärbt.
...
- **Kacheln sind nahtlos in jeder Anordnung.** Motive liegen vollständig in der Kachel, und der Rand besteht überwiegend aus der Grundfarbe. So treffen sich zwei Varianten immer auf Grundfarbe. Die Varianten werden gewichtet gestreut: ruhige Varianten häufig, auffällige selten (Grünhain-Gras 3 : 3 : 3 : 1, Erde 2 : 2 : 3 : 1).
...
Jedes Biom hat einen Grundton (Flächen), Akzente (seltene, lesbare Farben), eine Nachtfarbe (Ton der Dunkelheit bzw. Höhlenschwärze) und eine Grading-Absicht (Biom × Tageszeit × Wetter, §6.1 Pass 9). Gemeinsam genutzte Sprites sind in Grünhain-Farben gezeichnet; die Palettenzeile `biom_<id>` tönt `gras`, `erde`, `stein`, `holz` und `laub` in die Identität des Bioms (`assets-src/paletteRows.ts`, `BIOME_TINTS`). Eigene Biom-Tiles (Sand, Schnee, Asche, Kristall) zeichnen direkt in ihren Grundtönen.
...
| Salzküste (`salzkueste`) | 0 | `sand.3` `sand.2` `wasser.3` `stein.4` | `eis.3` Gischt und Salz · `laub.3` Tang und Koralle · `wasser.5` Brandung | `wasser.0` | Hell und luftig, leicht cyanfarbene Lichter, hoher Weißanteil, dunstige Ferne; Nacht marineblau mit silbernem Mondlicht auf dem Wasser. |
| Nebelmoor (`nebelmoor`) | 0 | `gras.1` `gras.2` `erde.1` `stein.2` | `gras.5` Moorgas und Irrlichter · `eis.2` Nebelschwaden · `laub.2` Moorbeeren | `gras.0` | Entsättigt, grünlich-grau, flacher Kontrast; Nebel hebt die Schatten an, Lichter wirken diffus; Nacht petrolschwarz. |
| Frostkamm (`frostkamm`) | 0 | `eis.3` `eis.2` `stein.2` `gras.1` | `laub.2` Frostbeeren · `wasser.4` Gletschereis · `eis.4` Glitzern | `eis.0` | Kalt, blaue Schatten, strahlendes Weiß, geringe Sättigung – Feuer wirkt hier besonders warm; Nacht klar und stahlblau. |
...
| Aschenschlund (`aschenschlund`) | 0 | `nacht.2` `stein.1` `stein.2` `nacht.3` | `feuer.3` `feuer.4` Lava und Glut · `sand.3` Schwefel | `feuer.0` | Dunkel und rauchig-entsättigt, Rot-Orange steigt aus der Tiefe, hoher Kontrast zwischen Asche und Glut; Ascheregen dämpft alles. |
...
| Wurzelhöhlen (`wurzelhoehlen`) | −1 | `erde.1` `erde.2` `holz.1` `gras.1` | `wasser.4` `wasser.5` Leuchtpilze · `sand.3` Harz und Bernstein | `erde.0` | Umgebungslicht ≈ 0, erdig-warmes Dunkel; Fackeln tragen die Szene, Pilze setzen türkise Inseln. |
| Tiefgrund (`tiefgrund`) | −2 | `stein.1` `stein.2` `nacht.3` `stein.0` | `eis.2` Kristalladern · `wasser.5` · `sand.3` Gold- und Erzglanz | `wasser.0` | Kalt und blaugrau, sparsam; Kristalle leuchten kühl, Erbauer-Ruinen stehen als helle Blöcke. |
