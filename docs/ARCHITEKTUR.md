...
- `FixedStepLoop`: Akkumulator, Schritt 1/60 s, max. 5 × ⌈Zeitfaktor⌉ Aufholschritte je Frame (Rest verworfen), `render(alpha)` mit Interpolationsfaktor. Zeitfaktor 0 … `MAX_TIME_SCALE` = 32: der Schlaf läuft ×30 (§11.5, `main.tsx` setzt Spieltempo × `session.timeScale` je Frame), Debug-`speed` bis ×32 (ADR-0034). Zeitquelle und Frame-Scheduler werden injiziert (Browser: `performance.now` + `requestAnimationFrame`; Tests: manuell).
...
- `createSimulation(config, options?)` (`src/game/setup.ts`) registriert in fester Reihenfolge (ADR-0028): 1 `world-chunks` (Aktive Zone, zuerst), 2 `motion`, 3 `world-collision` (gemerktes Kollisionsraster, meldet Tile-Änderungen an Zuhörer), 4 `player`, 5 `vitals` (Überlebenswerte und Temperatur nach der Bewegung), 6 `calendar`, 7 `weather-regions`, 8 `temperature`, 9 `inventory`, 10 `equipment` (eine gemeinsame `PlayerBags`), 11 `drops`, 12 `gathering`, 13 `interaction` (Fokus, E, Nutzungsziele), 14 `crafting`, 15 `tools` (`player.useItem`), 16 `light` (Lichtquellen, Lichtkarte), dann die Lebenssysteme 17 `conditions`, 18 `fear`, 19 `sleep`, 20 `actions`, 21 `skills`, 22 `death` (`addPlayerLifeSystems`: sie sehen jeden Treffer des Ticks; ein Kampfsystem kommt davor), 23 `cheats` (Cheats der Debug-Konsole `god`, `noclip`, `unlock`, ohne Tick-Haken; seine Schalter entstehen vor `player` und `vitals` und werden von Bewegung, Überlebenswerten und `PlayerHarm` gelesen, ADR-0034). Danach verbindet es die Haken (Einfluss- und Wärmequellen des Spielers, Lichtkarte → Furcht, Fertigkeiten ↔ Sammeln/Handwerk, Sitzplätze, Nutzungsziele, Sterben → Handwerks-Auftrag; docs/SPIEL.md §3), prüft, dass jeder Command einen Handler hat, und versiegelt die Aufhol-Registry. `options` reicht die Welt aus dem Welt-Worker und die Job-Queue des Streamings durch (`SimWorldOptions`).
...
- **Save-Versionen** (`src/save/versions.ts`, ADR-0030): Liste aller Versionen (1 = M3) mit den Teilnehmern und Datenversionen, die ein Build schreibt; die laufende Simulation muss genau die letzte schreiben. **Welt-Dump** (`src/save/dump.ts`): `exportWorld`/`importWorld` aller Datensätze einer Welt samt Save-Version, als Text kanonisches JSON. **Referenzspielstände** `tests/fixtures/saves/v<n>.json` (`npm run fixture:save`, `tools/save/fixture.ts`): Dump + Fakten des Spielers; `tests/unit/save/migrationen.test.ts` lädt jede Version in den aktuellen Build.
- `saveWorld(store, sim, { worldId, name, now, gameVersion, slot? })` schreibt in **einer** Transaktion Weltmeta, Weltrecord (Spielversion, Generator-Version, Diff-Format, Laufzeit-ID-Tabellen), genau die geänderten Chunks als Diffs (`saveChunkWorld`, `ChunkManager.collectChanges`/`markSaved`; hat der Speicher die Welt noch nicht, alle Abweichungen vom generierten Zustand, `forgetStorage`, ADR-0030) und den Snapshot-Slot; `loadWorld` prüft den Slot-Hash, liest die Chunk-Diffs (umnummeriert auf die aktuellen IDs), gibt sie dem Chunk-Store der neuen Simulation (`loadStored`) und stellt danach die Teilnehmer wieder her. Die Welt selbst wird nie gespeichert, sondern aus Seed und Größe neu erzeugt.
...
- `src/ui`: Preact + @preact/signals, DOM-Overlay über dem Canvas (`App.tsx` rendert den aktuellen Bildschirm, `mountApp` in `src/ui/index.ts`). Weltnahe UI im WebGL-Pass.
  ...
  - **Lesende Abtastungen der Sitzung** (ADR-0010, ADR-0035): Was die UI über Status und Events hinaus braucht, füllt `GameSession` in vom Aufrufer gehaltene Datensätze, ohne Allokation je Frame und ohne je Weltplan oder Welt zu erzeugen – `samplePlayer`, `sampleBags`, `sampleHud` (Brücke, HUD, Inventar), `sampleSky`, `mapChunk`, `startBeach`, `sampleGraves` (Minimap, Kompass). Kein UI-Modul liest `session.sim`.
...
- `tools/save/fixture.ts` (`npm run fixture:save`): Referenzspielstand der aktuellen Save-Version (ADR-0030).
- `tools/forbidden.ts` Verbotsliste · `tools/validate-content.ts` Content-Validator (Prüfungen in `tools/validator/checks.ts`, Zielwerte `tools/validator/zielwerte.json`; geplante Verwendungen und Erreichbarkeiten in `verwendungen-geplant.ts`/`reachability-geplant.ts`, ADR-0029) · `tools/shot.ts` Screenshots (Szenarien mit `viewports` je Auflösung, mit Prüfung von interner Größe und Pixelschärfe über `tools/lib/sharpness.ts`; die Weltkarten `weltkarte-klein|mittel|gross` zeichnet es in Node über `tools/world/weltkarte.ts`, ADR-0027) · `tools/bench.ts` Performance.
...
- **Systeme:** `world-chunks` (erstes System, global) bewegt je Tick die Zone zum Fokus – der Spieler auf seiner Ebene, ohne Spieler die gesteuerte Debug-Figur; ohne Fokus friert die Zone ganz ein – und gibt Chunks frei, die sie eingefroren hat und die keine Kamera hält (`ChunkManager.trim`; headless streamt niemand). Save-Teilnehmer `world-chunks`, `stateDigest` = Digest aller Chunk-Änderungen. `calendar` (der `Calendar` selbst), `weather-regions` (Welt-Tick, Teilnehmer) und `temperature` (Welt-Tick, zustandslos) sind global.
