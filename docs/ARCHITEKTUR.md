...
- `src/ui`: Preact + @preact/signals, DOM-Overlay über dem Canvas (`App.tsx` rendert den aktuellen Bildschirm, `mountApp` in `src/ui/index.ts`). Weltnahe UI im WebGL-Pass.
  ...
  - **Theme** (`src/ui/theme.ts`): Farb-Tokens `--dh-<name>` aus den UI-Farben der Master-Palette (`UI_HEX`), `--dh-ui-scale` ganzzahlig 1–4 (Auto = ⌊min(Breite/480, Höhe/270)⌋ in CSS-px, sonst Einstellung `accessibility.uiScale`). `base.css` verweist nur auf diese Tokens.
